...
get_latest_version() {
  ...
  local tmp_file
  tmp_file="$(mktemp)"
  if ! curl -sS -H "Accept: application/vnd.github.v3+json" -H "Authorization: token ${token}" -o "$tmp_file" 'https://api.github.com/repos/XTLS/Xray-core/releases/latest'; then
    "rm" "$tmp_file"
  ...
  if ! curl -sS -H "Accept: application/vnd.github.v3+json" -H "Authorization: token ${token}" -o "$tmp_file" 'https://api.github.com/repos/XTLS/Xray-core/releases'; then
    "rm" "$tmp_file"
    echo 'error: Failed to get release list, please check your network.'
    exit 1
  fi
